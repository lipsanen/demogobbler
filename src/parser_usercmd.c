...
#define READ_UINT(member, bits) out->member ## _exists = dg_bitstream_read_bit(&stream); \
    if(out->member ## _exists) out->member = dg_bitstream_read_uint(&stream, bits);

#define WRITE_UINT(member, bits) dg_bitwriter_write_bit(thisptr, input->member ## _exists ); \
    if(input->member ## _exists ) dg_bitwriter_write_uint(thisptr, input->member, bits);

#define READ_FLOAT(member) out->member ## _exists = dg_bitstream_read_bit(&stream); \
    if(out->member ## _exists) out->member = dg_bitstream_read_float(&stream);

#define WRITE_FLOAT(member) dg_bitwriter_write_bit(thisptr, input->member ## _exists ); \
    if(input->member ## _exists ) dg_bitwriter_write_float(thisptr, input->member);
...
dg_parse_result dg_parser_parse_usercmd(const dg_demver_data* version_data, const dg_usercmd *input, dg_usercmd_parsed* out) {
    ...
    out->buttons_exists = dg_bitstream_read_bit(&stream);
    if(out->buttons_exists) {
        out->buttons = dg_bitstream_read_sint32(&stream);
    }
    ...
    if(stream.overflow) {
        ...
        out->left_overbits = dg_bitstream_fork_and_advance(&stream, left);
    }
...
void dg_bitwriter_write_usercmd(dg_bitwriter* thisptr, dg_usercmd_parsed* input) {
    ...
    dg_bitwriter_write_bit(thisptr, input->buttons_exists);
    if(input->buttons_exists) {
        dg_bitwriter_write_sint32(thisptr, input->buttons);
    }
    ...
    dg_bitwriter_write_bitstream(thisptr, &input->left_overbits);
}
